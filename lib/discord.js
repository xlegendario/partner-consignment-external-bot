...
const EXTERNAL_CHANNEL_NAME = process.env.EXTERNAL_CHANNEL_NAME || "offer-inquiries";         // for Offer mode
const CONFIRM_CHANNEL_NAME  = process.env.CONFIRM_CHANNEL_NAME  || "confirmation-requests";  // for Confirm mode
...
export async function sendExternalOfferMessageGateway({
  ...
  const components = [{
    type: 1,
    components: [
      { type: 2, style: 3, label: `Accept Offer ${ourValue.split(" ")[0]}`, custom_id: `confirm_ext|${orderRecId}|${sellerId}|${inventoryRecordId}|${offerPrice ?? 0}` },
...
export async function sendExternalConfirmationMessageGateway({
  ...
  const { channelId } = await getChannelIdForSeller(sellerName || sellerId, "confirm");
  if (!channelId) throw new Error(`[Discord] No channel for seller="${sellerName || sellerId}" kind=confirm`);

  const components = [{
    type: 1,
    components: [
      { type: 2, style: 3, label: "Confirm", custom_id: `confirm_ext|${orderRecId}|${sellerId}|${inventoryRecordId}|${confirmPrice ?? 0}` },
      { type: 2, style: 4, label: "Deny",    custom_id: `deny_ext|${orderRecId}|${sellerId}|${inventoryRecordId}|${confirmPrice ?? 0}` }
    ]
  }];
