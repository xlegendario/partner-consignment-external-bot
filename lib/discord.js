...
export async function sendExternalOfferMessageGateway({
  ...
  const { channelId } = await getChannelIdForSeller(sellerName || sellerId, "offer");
  if (!channelId) throw new Error(`[Discord] No channel for seller="${sellerName || sellerId}" kind=offer`);
