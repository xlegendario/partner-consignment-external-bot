 ...
 * 3) als er dan nog steeds niets is → gooi een error (deal mag niet doorgaan)
 */
async function resolveConfirmedInventoryIdForExternal(exFields) {
  ...
  const sellerIdFromExt =
    getFirstLinkedId(
      exFields[FIELD_EXT_SELLER_LINK] ||
      exFields["Seller ID"] ||
      exFields["Linked Seller"]
    );
  ...
  if (!sellerId && !stockLevelId) {
    ...
    // → Hier hard stoppen, want we kunnen onmogelijk een betrouwbare Inventory record kiezen
    throw new Error(
      "No Inventory record found (missing Seller / Stock Levels link). Deal can not be processed."
    );
  }
  ...
  if (!foundInvId) {
    ...
    // → Ook hier hard stoppen: seller heeft het paar waarschijnlijk echt verwijderd
    throw new Error(
      "No Inventory record found for this External record (pair likely removed from seller inventory)."
    );
  }
