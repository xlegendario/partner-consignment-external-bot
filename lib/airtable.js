...
export async function setExternalConfirmation({
  ...
  statusName = "Confirmed",       // Offer Status single-select
  offerVatTypeLabel,              // "Margin" | "VAT0" | "VAT21"
  dealStatusName = "Closing",     // 👈 NEW: Deal Status single-select
}) {
  ...
  fields[FIELD_CONFIRMED_PRICE || "Confirmed Offer Price"] =
    typeof confirmedPrice === "number" ? Math.round(confirmedPrice * 100) / 100 : null;

  fields[FIELD_CONFIRMED_SELLER || "Confirmed Seller"] =
    confirmedSellerRecId ? [confirmedSellerRecId] : [];

  // Try single-select object form first, then fallback to plain string if needed
  try {
    fields[FIELD_OFFER_STATUS || "Offer Status"] = { name: statusName };
    if (offerVatTypeLabel) {
      fields[FIELD_OFFER_VAT_TYPE || "Offer VAT Type"] = { name: offerVatTypeLabel };
    }
    // 👇 NEW: set Deal Status = "Closing" on accept
    if (dealStatusName) {
      fields[FIELD_DEAL_STATUS || "Deal Status"] = { name: dealStatusName };
    }
    ...
    if (offerVatTypeLabel) {
      fields[FIELD_OFFER_VAT_TYPE || "Offer VAT Type"] = offerVatTypeLabel;
    }
    if (dealStatusName) {
      fields[FIELD_DEAL_STATUS || "Deal Status"] = dealStatusName;
    }
