...
const toText = (val) => {
  ...
  // Arrays (lookups often return arrays of primitives/objects)
...
const getSingleSelectLabel = (val) => {
  // Handles {name}, "VAT0", or [ {name}, "VAT0" ]
  if (val == null) return null;
  if (Array.isArray(val)) {
    for (const v of val) {
...
export async function createSalesFromExternal(ex) {
  ...
  // Robust VAT label extraction (works with select object, string, or lookup array)
  const vatName = getSingleSelectLabel(ex[FIELD_OFFER_VAT_TYPE || "Offer VAT Type"]);

  const fields = {
    [FIELD_SALE_PRODUCT_NAME]: toText(ex[FIELD_PRODUCT_NAME || "Product Name"]) || null, // lookup → plain text
  ...
  try {
    ...
    // Fallback: some bases only accept plain string for single select via API
    if (vatName) fields[FIELD_SALE_VAT_TYPE] = vatName;
