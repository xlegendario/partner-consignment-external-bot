...
export async function createSalesFromExternal(ex) {
  ...
  const fields = {
    [FIELD_SALE_PRODUCT_NAME]: ex[FIELD_PRODUCT_NAME || "Product Name"] || null,
    [FIELD_SALE_SKU]:          skuId ? [skuId] : [],
    [FIELD_SALE_SIZE]:         ex[FIELD_SIZE || "Size"] || null,
    [FIELD_SALE_BRAND]:        ex[FIELD_BRAND || "Brand"] || null,
  ...
  // Try with single-select object; if that fails, retry as raw string
  ...
  try {
    ...
    if (vatName) {
      fields[FIELD_SALE_VAT_TYPE] = vatName;
    }
